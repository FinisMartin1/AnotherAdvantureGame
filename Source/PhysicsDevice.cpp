...
{
	world = new b2World(GV2PV(gravity));
...
//**************************************
{
	...
	case BodyShape::Rectangle:
		...
		pShape.SetAsBox(RW2PW(presets.physics.center.x/2.0f), RW2PW(presets.physics.center.y/2.0f));
...
//**************************************
{
	...
	Vector2D position;
	position.x = PW2RW(bpos.x);
	position.y = PW2RW(bpos.y);
